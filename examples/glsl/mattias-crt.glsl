...
void mainImage(const in vec4 inputColor, const in vec2 fragCoord, out vec4 fragColor) {
	...
	col.r = texture(inputBuffer,vec2(x+uv.x+0.0004,uv.y+0.0004)).x+0.05;
	col.g = texture(inputBuffer,vec2(x+uv.x+0.0000,uv.y-0.0008)).y+0.05;
	col.b = texture(inputBuffer,vec2(x+uv.x-0.0008,uv.y+0.0000)).z+0.05;
	col.r += 0.08*texture(inputBuffer,0.75*vec2(x+0.025, -0.027)+vec2(uv.x+0.001,uv.y+0.001)).x;
	col.g += 0.05*texture(inputBuffer,0.75*vec2(x+-0.022, -0.02)+vec2(uv.x+0.000,uv.y-0.002)).y;
	col.b += 0.08*texture(inputBuffer,0.75*vec2(x+-0.02, -0.018)+vec2(uv.x-0.002,uv.y+0.000)).z;

	col = clamp(col*0.6+0.4*col*col*1.0,0.0,1.0);

	float vig = (0.0 + 1.0*16.0*uv.x*uv.y*(1.0-uv.x)*(1.0-uv.y));
	col *= vec3(pow(vig,0.16));
	...
	col*=1.0-0.65*vec3(clamp((mod(fragCoord.x, 2.)-1.0)*2.0,0.0,1.0));
	
 
	// Remove the next line to stop cross-fade between original and postprocess
	// float comp = smoothstep( 0.1, 0.9, sin(time) );
	// vec3 oricol = texture( inputBuffer, vec2(q.x,q.y) ).xyz;
